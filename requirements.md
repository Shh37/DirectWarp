...
4. 候補数 N（Custom Search API の取得件数。デフォルト: N=5）、Gemini モデル（既定: gemini-1.5-flash）、タイムアウト、API キーは設定画面で指定可能。
...
- トリガー文字列のカスタマイズ
- ダーク/ライトテーマの切り替え
- サイトジャンル選好設定（各ジャンルごとに「嫌う/通常/好む」を選択可能）
  - 初期設定では以下のジャンルを用意:
    - Wiki系サイト（例: Wikipedia、Fandom、Wikiwandなど）
    - 政府・行政サイト（例: .go.jp ドメイン、政府統計など）
  - 選好設定はGeminiがURLを評価する際の重み付けに反映
  - 将来的にジャンルの追加・削除が可能な仕組みを検討
- 設定のリセット機能

### 3. サイトジャンル選好の動作仕様
- 各ジャンルに対して以下の3つの選好を設定可能:
  - 嫌う: 該当ジャンルのサイトを検索結果から除外するようGeminiに指示
  - 通常: ジャンルによるフィルタリングを行わない（デフォルト）
  - 好む: 該当ジャンルのサイトを優先的に表示するようGeminiに指示
- 選好設定はGeminiへのプロンプトに反映され、検索結果の選定に影響
- 複数のジャンル設定が競合する場合は、より具体的な設定が優先される
